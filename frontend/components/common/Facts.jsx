...
export default function Facts({ cmsContent }) {
  const yearsMax = Number(cmsContent?.yearsCounterMax) || 25;
  const yearsTitleLine1 = cmsContent?.yearsTitleLines?.[0] || "Years Of";
  const yearsTitleLine2 = cmsContent?.yearsTitleLines?.[1] || "experience";
  const yearsParagraph =
    cmsContent?.yearsParagraph ||
  ...
  const counterItems =
    Array.isArray(cmsContent?.counters) && cmsContent.counters.length
      ? cmsContent.counters
      : counters;
  return (
            ...
            <div className="year-of-expariance-wrapper bg-blur-style-one tmp-scroll-trigger tmp-fade-in animation-order-1">
              <div className="year-expariance-wrap">
                ...
                <h2 className="counter year-number">
                  <OdometerComponent max={yearsMax} />
                </h2>
              ...
              <p className="year-para">{yearsParagraph}</p>
            </div>
          </div>
          <div className="col-12 col-lg-6 col-xl-6 col-xxl-6">
